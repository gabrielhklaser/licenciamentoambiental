## [2026-09-26] RTT/RRT DO CAU/BR, CONFRONTO BILATERAL DE DOCUMENTOS, TRs SEM APLICAÇÃO CRUZADA E LEITURA DE CAMADAS GIS

- **Arquivos:** `licenciamento/identificador_documentos.py`, `licenciamento/validador_documentos.py`, `licenciamento/leitor_gis.py` (NOVO), `licenciamento/agente_gis.py` (NOVO), `licenciamento/auditor_tecnico.py`, `licenciamento/agente_administrativo.py`, `licenciamento/auditor_sistema.py`, `app.py`, `config/aprendido_documentos.json`, `tests/test_conferencias_rtt_gis.py` (NOVO - 31 testes).
- **O quê:**
  1. **RTT/RRT são SEMPRE do CAU/BR e de Arquitetos/Urbanistas** - novo par de constantes/expressões (`TIPO_RRT`/`TIPO_ART`, `RE_CAU`/`RE_CREA`/`RE_CRBIO`, `RE_REGISTRO_CAU`/`RE_CREA`/`RE_CRBIO`) e helpers `conselho_do_texto`, `conselho_do_registro` ('A67154-1' -> CAU/BR, 'RS233891' -> CREA, '110544/03-D' -> CRBio) e `tipo_registro_por_conselho`. `PADROES_TIPO["ART"]` passou a ser exclusivo de CREA/CRBio e `"RRT"` ganhou as palavras do CAU/BR - um RTT nunca mais é classificado como ART de engenheiro/geólogo;
  2. **VÁRIAS RTTs no mesmo processo** - `AuditorTecnico.identificar_registros_rt` devolve TODOS os registros do arquivo (dedupe por número, janela a partir do nº encontrado para não herdar a atividade da RTT anterior) e `ValidadorDocumentos.extrair_rtts` passa a devolver número, profissional, conselho, registro no CAU e a ETAPA de cada uma (Projeto Urbanístico, Execução de Obras...);
  3. **Registro identificado com parcimônia** - `identificar_art_rtt` exige título do registro ("anotação/registro de responsabilidade técnica") OU conselho profissional + número, e devolve `{numero, nome, tipo, orgao, registro, atividade}`; um laudo que apenas cita "com ART de responsável técnico" continua NÃO sendo ART;
  4. **Confronto BILATERAL formulário x documentos emitidos** - novas conferências no `AgenteAdministrativo`: `_conferir_matricula` (nº declarado x nº do documento), `_conferir_contrato_social` (CNPJ e razão social, com tolerância a LTDA/ME/EPP) e `_conferir_rtts_anexadas` (sentido documento -> formulário: RTT/ART anexada e NÃO declarada na 4.3/item 14 é pendência). Resultado e resumo ganharam `conferencia_matricula`, `conferencia_contrato_social`, `conferencia_rtts_anexadas`, `rtts_anexadas_nao_declaradas` e `divergencias_documentais`;
  5. **Conselho incompatível nunca vira CONFORME** - `_conferir_art_com_formulario` cruza o conselho do documento com o declarado (número do registro) e devolve `REVISAO_MANUAL` com `metricas["conselho_incompativel"]`; `_conferir_responsaveis_etapas` guarda `tipo`/`conselho_declarado`/`conselho_documento` e procura a ART/RTT PRIMEIRO nos documentos que são registro (fallback transparente com nota no `nivel`), com casamento de número por fronteira (sem falso positivo de substring);
  6. **TRs sem aplicação cruzada** - `_tr_esperado_pelo_titulo` passa a decidir pela ocorrência MAIS CEDO do cabeçalho (empate: frase mais específica; sigla só no início): um Laudo de Cobertura Vegetal que cita "sondagem" e "reposição florestal" recebe SÓ o TR de cobertura vegetal, um Inventário de Fauna que descreve a cobertura vegetal recebe SÓ o TR de fauna, e um Laudo Geológico que avista fauna recebe SÓ o TR de meio físico. A rota de projeto urbanístico passou a verificar o registro ANTES (uma RRT cuja atividade é "Projeto Urbanístico" é registro, não projeto) e as camadas GIS retornam `[]` sem TR algum;
  7. **Ensaios de infiltração** - `extrair_parametros_sondagem` conta "ensaios de infiltração" (inclusive "duplo anel") como ensaios de permeabilidade e extrai "profundidade de investigação de X m", "sondagens ... até X m" e "atingindo X m de profundidade";
  8. **Leitura de camadas GIS (skill gis-multicamadas)** - `licenciamento/leitor_gis.py` (NOVO, stdlib-only) lê KML/KMZ/GeoJSON/GPX/WKT em camadas temáticas, normaliza SRS (SIRGAS 2000 -> EPSG:4674, SIRGAS 2000 UTM 22S -> EPSG:31982, WGS84 -> EPSG:4326), calcula área (Gauss em graus + conversão), distância Haversine e ponto-em-polígono, e converte UTM do formulário (fuso 22S) para geográfica com série de Snyder; `licenciamento/agente_gis.py` (NOVO) confronta área da camada x área declarada, contenção do ponto do empreendimento e distância a corpos hídricos/APP - e NENHUM resultado carrega TR de conteúdo (`metricas["tr_aplicado"] = None`);
  9. **Painel** - `app.py` aceita `.kml/.kmz/.geojson/.gpx` no upload, envia as camadas ao `AgenteGIS` (usando a coordenada UTM ou geográfica do formulário) e exibe o bloco "Camadas GIS - confronto geométrico"; a conferência de matrícula/contrato social e as RTTs não declaradas aparecem na triagem administrativa;
  10. **Checklist** - a exigência oficial "21. Arquivo KMZ/KML e DWG do projeto urbanístico e APPs" passou a ser tipada como `CAMADA_GIS` (antes virava projeto urbanístico) e `casar_exigencia` ganhou o PAR pelo tipo: bônus quando o anexo é do tipo exigido e veto quando não é - um `.kml` nunca atende à exigência de um laudo e vice-versa. Correção também do nome do arquivo: tokens curtos decisivos (`rrt`, `rtt`, `art`, `kmz`, `kml`...) voltaram a ser considerados e a extensão geoespacial passou a contar ("RTT - Projeto Urbanistico.pdf" é RRT, não projeto);
  11. **Auditor do sistema** - `verificar_conferencias_documentais` ganhou as checagens CONF-RRT-TIPO, CONF-RRT-MULTIPLAS, CONF-RRT-CONSELHO, CONF-RRT-ATIVIDADE, CONF-CONSELHO-DIVERGENTE, CONF-TR-CRUZADO, CONF-GIS-LEITURA, CONF-GIS-AREA, CONF-GIS-TR e CONF-GIS-AREA-DIVERG (bateria viva anti-regressão).
- **Por quê:** Instrução do licenciador (26/09/2026): RTT/RRT pertencem sempre ao CAU/BR e a Arquitetos/Urbanistas; podem existir VÁRIAS RTTs no processo; o confronto formulário x documentos emitidos deve ser rigoroso e bilateral; e os TRs não podem ser aplicados de forma cruzada em arquivos errados. As camadas GIS são exigência oficial do checklist de LP/LI (item 21).
- **Como reverter:** `git revert <hash deste commit>`.
- **Estado:** 158 testes OK (100% verde) e bateria do `AuditorSistema` sem achados alto/crítico.
